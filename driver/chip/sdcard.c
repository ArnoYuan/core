...
#ifdef SD_DEBUG
#define sd_dbg(fmt, ...)   kernel_printf(fmt, ##__VA_ARGS__)
#else
#define sd_dbg(fmt, ...)
#endif
...
int sd_wait_ready(struct sdcard* sd)
{
  ...
  do{
    ...
    if(data == 0xFF)
      return 0;
  }while(pi_get_millis() - lasttime < SD_TIMEOUT);
...
int sd_check_response(struct sdcard* sd, pi_uint8_t rep)
{
  pi_assert(sd);
  pi_assert(sd->spi);
...
int sd_write_block(struct sdcard* sd, pi_uint8_t cmd, pi_uint8_t* wr_data)
{
  pi_assert(sd);
  pi_assert(sd->spi);
  ...
  if(cmd != 0xFD)
  {
    ...
    pi_uint16_t dummy = 0xFFFF;

    spi_write(sd->spi, (pi_uint8_t*)&dummy, 2);
...
int sd_read_data(struct sdcard* sd, pi_uint8_t* rd_data, pi_size_t rd_len)
{
  pi_assert(sd);
  pi_assert(sd->spi);
  pi_assert(rd_data);
...
int sd_write_command(struct sdcard* sd, pi_uint8_t cmd, pi_uint32_t args, pi_uint8_t crc)
{
  pi_assert(sd);
  pi_assert(sd->spi);
  pi_uint8_t r1 = 0;
...
int sd_get_sector_count(struct  sdcard* sd)
{
  ...
  if((csd[0] & 0xC0) == 0x40)
  {
    csize = csd[9] + ((pi_uint16_t)csd[8] << 8) + 1;
    capacity = (pi_uint32_t)csize << 10;
  }
  else
  {
    n = (csd[5] & 15) + ((csd[10] & 128) >> 7) + ((csd[9] & 3) << 1) + 2;
    csize = (csd[8] >> 6) + ((pi_uint16_t)csd[7] << 2) + ((pi_uint16_t)(csd[6] & 3) << 10) + 1;
    capacity = (pi_uint32_t)csize << (n - 9);
  }
...
int sd_initialize(struct sdcard* sd)
{
  ...
  int r1 = 0;
  pi_uint16_t retry;
  pi_uint8_t rd_data[4];
  spi_set_speed(sd->spi, SD_LOW_SPEED);

  pi_uint8_t dummy = 0xFF;
